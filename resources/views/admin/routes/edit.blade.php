            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <div class="flex justify-between items-center mb-6">
                        <h2 class="text-2xl font-semibold">Редактировать маршрут</h2>
                    ...
                    <form action="{{ route('admin.routes.update', $route) }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <x-custom.forms.input type="text" id="name" name="name" :value="old('name', $route->name)" required />
                            @error('name')
                                ...
                                @foreach($attractions as $attraction)
                                    <label class="flex items-start p-4 border rounded-lg hover:bg-gray-50">
                                        <input type="checkbox" name="attractions[]" value="{{ $attraction->id }}"
                                               ...
                                               {{ in_array($attraction->id, old('attractions', $route->attractions->pluck('id')->toArray())) ? 'checked' : '' }}>
                                        <div class="ml-3">
